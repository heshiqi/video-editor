...
int open_input_file(const char *filename,
    ...
    /* Find a decoder for the audio stream. 获取音频的解码器 */
    ...
    /* Allocate a new decoding context. 创建解码器上下文 */
    ...
    /* Initialize the stream parameters with demuxer information. 初始化流参数 */
    ...
    if ((error = av_read_frame(input_format_context, &input_packet)) < 0) {
        ...
        if (error == AVERROR_EOF)
            *finished = 1;
            ...
            LOGE("Could not read frame (error '%s')\n", av_err2str(error));
            return error;
        }
    }
...
static int encode_audio_frame(AVFrame *frame,
    ...
    if (error == AVERROR_EOF) {
        ...
        LOGE(stderr, "Could not send packet for encoding (error '%s')\n",av_err2str(error));
        return error;
    }
    ...
    if (error == AVERROR(EAGAIN)) {
        ...
        LOGE(stderr, "Could not encode frame (error '%s')\n",av_err2str(error));
        goto cleanup;
...
int mainddd(char *musicPath, char *outPath) {
    ...
    if (open_input_file(musicPath, &input_format_context, &input_codec_context))
        goto cleanup;
    /* Open the output file for writing. */

    if (open_output_file(outPath, input_codec_context,
...
Java_com_h_arrow_medialib_videoeditor_VideoEditor_testAcc(JNIEnv *env, jobject instance,
    ...
    LOGE("dskdjfjldjflsdjlf--------");
    const char *musicPath = (*env)->GetStringUTFChars(env,infile_,0);
    const char *outPath = (*env)->GetStringUTFChars(env,outFile_, 0);
    av_register_all();
    LOGE("dskdjfjldjflsdjlf-------- %s",outPath);
    mainddd(musicPath,outPath);
}
